...
function createCards(parentElement, shuffledImageClasses) {
    ...
    for (i = 0; i < 12; i++) {
        let temp = appendNewCard(parentElement);
        temp.classList.add(shuffledImageClasses[i]);
        let obj = {
            index: i,
            element: temp,
...
function incrementCounter(counterName, parentElement) {
    ...
    if (parentElement == "flip-count") {
        parentElement = document.getElementById("flip-count");
    } else if (parentElement == "match-count") {
...
function onCardFlipped(newlyFlippedCard) {
    ...
    incrementCounter("flips", "flip-count");
    ...
    incrementCounter("matches", "match-count");
...
function resetGame() {
    ...
    /* Step 2: Clear all the cards by using a while loop to remove the first child of the card container as long as a first child exists. See "To remove all children from an element:" in the Examples section of the MDN removeChild documentation -> https://mzl.la/3bklFxP */
